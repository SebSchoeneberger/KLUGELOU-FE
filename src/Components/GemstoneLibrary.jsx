...
const GemstoneLibrary = ({
  ...
  const filteredStones = stones.filter((stone) => {
    ...
    const matchesCrystalSystem = selectedCrystalSystem
      ? stone.crystalSystem.toLowerCase() ===
        selectedCrystalSystem.toLowerCase()
      : true;
  ...
  const currentStones = filteredStones.slice(
    indexOfFirstStone,
    indexOfLastStone
  );
  ...
  return (
    <>
      <section
        className={`flex flex-col items-center justify-center pb-24 ${backgroundClass}`}
      >
        <h1
          id="gemstone-library"
          className="font-nunito text-4xl font-medium text-customBlack text-center pt-24 pb-20"
        >
          {title}
        </h1>
